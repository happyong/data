...
public class TFundDaoService extends BaseDaoService
{
	...
	{
		if (WebUtil.empty(code)) return true;
	...
	public List<TSymbol> getSymbols(String cond)
	{
		...
		for (Map<String, Object> result : results) list.add(new TSymbol().fromDbMap(TSymbol.class, result));
		return list;
	}
	...
	private static final String insert_t_quote_daily = "insert into t_quote_daily (code, date, time, price, close_prev, open, high, low, volume, amount, high_60, low_60, volume_60) values "
			+ "(:code, :date, :time, :price, :closePrev, :open, :high, :low, :volume, :amount, :high60, :low60, :volume60) ";
	...
	{
		if (WebUtil.empty(code) || WebUtil.empty(date)) return true;
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("code", code);
		map.put("date", date);
	...
	public List<TQuoteDaily> getQuoteDailys(String cond)
	{
		...
		sql = WebUtil.substituteParam("cond", cond, sql);
		List<Map<String, Object>> results = query(sql);
		for (Map<String, Object> result : results) list.add(new TQuoteDaily().fromDbMap(TQuoteDaily.class, result));
		return list;
	}
	...
	public List<TNetDaily> getNetDailys(String cond)
	{
		...
		sql = WebUtil.substituteParam("cond", cond, sql);
		List<Map<String, Object>> results = query(sql);
		for (Map<String, Object> result : results) list.add(new TNetDaily().fromDbMap(TNetDaily.class, result));
		return list;
	}
