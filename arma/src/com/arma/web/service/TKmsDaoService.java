...
public class TKmsDaoService extends BaseDaoService
{
    ...
    private static final String insert_t_keytype = "insert into t_keytype (name_cn, name_en, demo_cn, demo_en, update_date) values (:nameCn, :nameEn, :demoCn, :demoEn, :updateDate) ";
    ...
    private static final String update_t_keytype = "update t_keytype set name_cn=:nameCn, name_en=:nameEn, demo_cn=:demoCn, demo_en=:demoEn, update_date=:updateDate where type_id=:typeId ";
    ...
    public List<TKeytype> getKeytypes(String cond)
    {
        ...
        if (WebUtil.empty(cond)) cond = "1=1";
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKeytype().fromDbMap(TKeytype.class, result));
        return list;
    }
    private static final String select_t_keytype = "select type_id as typeId, name_cn as nameCn, name_en as nameEn, demo_cn as demoCn, demo_en as demoEn, update_date as updateDate from t_keytype where ${cond} ";
    ...
    public List<TKeyenum> getKeyenums(String cond)
    {
        ...
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKeyenum().fromDbMap(TKeyenum.class, result));
        return list;
    }
    ...
    private static final String insert_t_keyword = "insert into t_keyword (content, name_cn, name_en, type_id, as_enum, update_date) values (:content, :nameCn, :nameEn, :typeId, :asEnum, :updateDate) ";
    ...
    private static final String update_t_keyword = "update t_keyword set content=:content, name_cn=:nameCn, name_en=:nameEn, type_id=:typeId, as_enum=:asEnum, update_date=:updateDate where key_id=:keyId ";
    ...
    public List<TKeyword> getKeywords(String cond)
    {
        ...
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKeyword().fromDbMap(TKeyword.class, result));
        return list;
    }
    private static final String select_t_keyword = "select key_id as keyId, content as content, name_cn as nameCn, name_en as nameEn, type_id as typeId, as_enum as asEnum, update_date as updateDate from t_keyword where ${cond} ";
    ...
    public List<TKeyrank> getKeyranks(String cond)
    {
        ...
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKeyrank().fromDbMap(TKeyrank.class, result));
        return list;
    }
    ...
    public List<TKnowledge> getKnowledges(String cond)
    {
        ...
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKnowledge().fromDbMap(TKnowledge.class, result));
        return list;
    }
    private static final String select_t_knowledge = "select km_id as kmId, km_uuid as kmUuid, ckey_id as ckeyId, content as content, update_date as updateDate from t_knowledge where ${cond} ";
    ...
    public List<TKnowkey> getKnowkeys(String cond)
    {
        ...
        sql = WebUtil.substituteParam("cond", cond, sql);
        List<Map<String, Object>> results = query(sql);
        for (Map<String, Object> result : results) list.add(new TKnowkey().fromDbMap(TKnowkey.class, result));
        return list;
    }
    ...
    {
        if (bean == null || bean.empty() || kmId0 < 1) return 0;
    ...
    {
        if (minutes == 0 || WebUtil.empty(cond)) return 0;
    ...
    {
        if (WebUtil.empty(cond)) return new ArrayList<Integer>();
    ...
    {
        if (ckeyId < 1) return new ArrayList<Integer>();
        if (WebUtil.empty(tkkCond)) return getKmIds(false, "ckey_id=" + ckeyId + " order by km_id ");
    ...
    {
        if (WebUtil.empty(cond)) return new ArrayList<Map<String, Object>>();
    ...
    private List<Integer> ids(String key, String sql)
    {
        ...
        List<Map<String, Object>> results = query(sql);
        for (int i = 0; i < results.size(); i++) rets.add((Integer)results.get(i).get(key));
        return rets;
    }
