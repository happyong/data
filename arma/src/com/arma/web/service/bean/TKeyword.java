...
public class TKeyword extends BaseDaoBean
{
    ...
    {
        return (keyId + "|" + WebUtil.unull(content) + "|" + WebUtil.unull(nameCn) + "|" + WebUtil.unull(nameEn) + "|" + typeId + "|" + asEnum + "|" + DateUtil.date24Str(
                updateDate, DateUtil.df_long));
    }
    ...
    {
        xop.openTag("keyword", InVarKM.attrs_keyword,
                new String[] { "" + getKeyId(), getContent(), getNameCn(), getNameEn(), "" + getTypeId(), "" + isAsEnum(), DateUtil.str(getUpdateDate()) });
        if (close)
            xop.closeTag();
    }
