	...
	{		
		...
		if ("sql".equals(type))
		{
			html = (WebUtil.empty(p1) ? SkyUtil.getHtml4SqlPage(address) : 
				SkyUtil.getHtml4SqlContent(p1, address, GlobalCache.getInstance().getBean(TFundDaoService.class)));
			success = true;
		}
		else if ("file".equals(type))
		{
			...
			else
			{
				if (p1.startsWith("{serverlog}"))
				{
					...
					if (FileUtil.exist(1, log) == null) log = WebUtil.path(GlobalCache._config_root + "/../logs/server.log");
