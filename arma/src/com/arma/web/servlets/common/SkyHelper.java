...
public class SkyHelper
{
    ...
    public static boolean sky(String servlet, HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (!ConfigHelper.NAME_SKY_ADMIN.equals(servlet) || WebUtil.empty(type))
            return false;
        ...
        if ("sql".equals(type))
        {
            html = (WebUtil.empty(p1) ? SkyUtil.getHtml4SqlPage(address) : SkyUtil.getHtml4SqlContent(p1, address, GlobalCache.getInstance().getBean(TFundDaoService.class)));
            success = true;
        }
        else if ("file".equals(type))
        {
            ...
            {
                if (p1.startsWith("{serverlog}"))
                {
                    String log = (WebUtil.unix() ? "" : "c:/neulion/logs/server.log");
                    if (FileUtil.exist(1, log) == null)
                        log = WebUtil.path(GlobalCache._config_root + "/../logs/server.log");
                    p1 = WebUtil.substituteName("{serverlog}", log, p1);
                    absolute = true;
                }
    ...
    public static void system(String format, Map<String, Object> map)
    {
        ...
        xop.appendTag(false, "build4SavannaCommon", WebUtil.build(null, builds.get("savanna-common.jar")), null, null);
    ...
    {
        _logger.info("sky server shutdown, user|" + user + "|ip|" + ip);
        new Thread()
        {
            public void run()
            {
