...
public class FundCacher
{
    ...
    public static void setup()
    {
        ...
        {
            if (!cur_codes.contains(bean.getCode()))
                cur_codes.add(bean.getCode());
        ...
        {
            if (_quotes_daily.containsKey(code) || _symbols.get(code).getType() == InVarAM.i_fund_type + 2)
                continue;
            quotes = dao.getQuoteDailys("code='" + code + "' order by code, date desc limit 1");
            if (quotes.size() > 0)
                putQuoteDaily(quotes.get(0));
            else
                _logger.info("setup: no quote daily - " + _symbols.get(code).toText());
        }
        ...
        for (String code : _symbols.keySet())
        {
            if (_nets_daily.containsKey(code))
                continue;
            nets = dao.getNetDailys("code='" + code + "' order by code, date desc limit 1");
            if (nets.size() > 0)
                putNetDaily(nets.get(0));
            else
                _logger.info("setup: no net daily - " + _symbols.get(code).toText());
        }
    ...
    private static void putQuoteDaily(TQuoteDaily bean)
    {
        ...
        if (cur == null || cur.getDate().compareTo(bean.getDate()) < 0)
            _quotes_daily.put(code, bean);
    }
    ...
    public static void flush()
    {
        ...
        if (!_last_trade.equals(last_trade))
        {
            ...
            if (_is_trade_day && InVarAM._b_scan_symbols_setup)
                flushSymbols();
    ...
    {
        for (TSymbol bean : symbols.values())
            if (bean.hit(focus))
                return bean;
        return null;
    }
    ...
    private static String[] getHqList(boolean quote)
    {
        if (_hq_quote == null || _hq_net == null)
        {
            ...
            for (TSymbol bean : _symbols.values())
            {
                ...
                if (bean.getType() == InVarAM.i_fund_type + 2)
                    continue;
                i_quote++;
    ...
    {
        if (!_need_quotes_daily_flush || _finish_quotes_daily_flush)
            return;
        ...
        {
            if (quotes_daily == null)
                quotes_daily = new HashMap<String, TQuoteDaily>();
            for (String list : getHqList(true))
                new FundSpider().scanSinaQuoteJs(list, quotes_daily);
        }
        ...
        for (String key : quotes_daily.keySet())
        {
            ...
            if (key.compareTo(last.key(_last_trade)) < 0)
                continue;
        ...
        for (TQuoteDaily bean : _quotes_daily.values())
            if (!bean.normalize())
                quotes.add(bean);
        normalizeQuoteDaily(quotes);
    }
        ...
        for (TQuoteDaily bean : quotes)
        {
            ...
            if (code_last != null && !code_last.equals(bean.getCode()))
            {
                ...
                if (count >= 300)
                {
                    ...
                    for (String key : keys)
                        normalizeQuoteDaily(key, quotes_daily, update_params);
                    count = 0;
                    sb = new StringBuffer();
                    keys.clear();
                    quotes_daily.clear();
            ...
            if (date_s == null || date_s.compareTo(bean.getDate()) > 0)
                date_s = bean.getDate();
            if (date_e == null || date_e.compareTo(bean.getDate()) < 0)
                date_e = bean.getDate();
            if (code_last == null)
                code_last = bean.getCode();
        }
    ...
    private static void normalizeQuoteDaily(String key, Map<String, TQuoteDaily> quotes_daily, List<Map<String, Object>> update_params)
    {
        ...
        {
            if (key2.compareTo(start_key) < 0 || key2.compareTo(key) > 0)
                continue;
            bean = quotes_daily.get(key2);
            if (bean == null)
                continue;
            if (h60 < 0d || h60 < bean.getHigh())
                h60 = bean.getHigh();
            if (l60 < 0d || l60 > bean.getLow())
                l60 = bean.getLow();
            if (bean.getVolume() < 0d)
                continue;
            count++;
            v60 += bean.getVolume();
        }
        ...
        if (success)
        {
            ...
            _logger.info("quote daily normalize " + (success ? "done" : "failed") + ", new [" + _symbols.get(quote_daily.getCode()).getNameS() + "|" + quote_daily.toText()
                    + "], cur [" + text + "]");
        }
    }
    ...
    public static void flushNetDaily()
    {
        ...
        for (String list : getHqList(false))
            new FundSpider().scanSinaNetJs(list, nets_daily);
        ...
        for (TNetDaily bean : _nets_daily.values())
            if (!bean.normalize())
                nets.add(bean);
        normalizeNetDaily(nets);
    }
        ...
        for (TNetDaily bean : nets)
        {
            ...
            if (code_last != null && !code_last.equals(bean.getCode()))
            {
                ...
                if (count >= 300)
                {
                    ...
                    for (String key : keys)
                        normalizeNetDaily(key, nets_daily, update_params);
                    count = 0;
                    sb = new StringBuffer();
                    keys.clear();
                    nets_daily.clear();
    ...
    private static void normalizeNetDaily(String key, Map<String, TNetDaily> nets_daily, List<Map<String, Object>> update_params)
    {
        ...
        for (String key2 : nets_daily.keySet())
        {
            ...
            if (ph5 < -101d || ph5 < price)
                ph5 = price;
            price = premium(3, null, null, net_daily, symbol);
            if (pl5 < -101d || pl5 > price)
                pl5 = price;
        }
    ...
    {
        if (net_daily == null || net_daily.empty() || symbol == null || symbol.empty())
            return -200d;
        ...
        else
        {
            ...
            if (quote_daily != null)
                return (price(type, quote_daily, quote_rt) - net) * 100d / net;
        }
        return -200d;
    }
    ...
    {
        return (type == 0 || type == 1 ? quote_daily.getPrice() : (type == 2 ? quote_daily.getHigh() : (type == 3 ? quote_daily.getLow() : (quote_rt == null ? -200d : (type == 4
                || type == 5 ? quote_rt.getPrice() : (type == 6 ? quote_rt.getHigh() : quote_rt.getLow()))))));
    }

    public static boolean needUpdateRtStats()
    {
        if (_is_rt_stats_first)
            return true;
        if (!_is_trade_day)
            return false;
        ...
        if (_is_trade_day && InVarAM.s_trade_times[5].compareTo(now_time) < 0)
            _need_quotes_daily_flush = true;
    ...
    public static void flushRtStats()
    {
        ...
        for (String list : getHqList(true))
            new FundSpider().scanSinaQuoteJs(list, quotes_rt);
        ...
        for (String key : quotes_rt.keySet())
        {
            ...
            if (key.compareTo(quote_rt.key(_last_trade)) < 0 || cur_codes.contains(quote_rt.getCode()))
                continue;
            str = quote_rt.getDate() + "T" + quote_rt.getTime();
            if (time == null || time.compareTo(str) < 0)
                time = str;
            symbol = normalizeRtStats(key, quotes_rt);
            if (symbol == null)
                continue;
            for (int i = 0; i < InVarAM.s_names.length; i++)
                if (!cur_codes.contains(symbol.code(i)))
                    cur_codes.add(symbol.code(i));
        }
        if (_is_rt_stats_first)
            _is_rt_stats_first = false;
    ...
    private static TSymbol normalizeRtStats(String key, Map<String, TQuoteDaily> quotes_rt)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            if (symbols[i] == null || net_daily[i] == null)
                continue;
            ...
            if (i < 2)
            {
                ...
                if (quote_daily[i] == null || quote_rt[i] == null)
                    continue;
    ...
    public static List<String> filter(String symbol)
    {
        ...
        if (!WebUtil.empty(symbol))
        {
            ...
            for (int i = 1; i < arr.length; i++)
            {
                ...
                if (!includes.contains(arr[i]) && !excludes.contains(arr[i]))
                    list.add(arr[i]);
            }
        }
        for (TSymbol bean : _symbols.values())
            if (bean.filter(includes, excludes) && !codes_b.contains(bean.code(1)))
                codes_b.add(bean.code(1));
        return codes_b;
    }
    ...
    public static int compare(boolean asc, int sort_type, String code_b1, String code_b2)
    {
        ...
        if (sort_type == 111)
            ret = ret2;
        else if (sort_type == 112)
            ret = comparte(s1.getNameS(), s2.getNameS());
        else if (sort_type == 113)
            ret = comparte(s1.getStartDate(), s2.getStartDate());
        else if (sort_type == 114)
            ret = comparte(s1.getManager(), s2.getManager());
        ...
        else
        {
            ...
            if (stat1 == null && stat2 == null)
                return (asc ? ret2 : 0 - ret2);
            else if (stat1 == null)
                return 1;
            else if (stat2 == null)
                return -1;

            if (sort_type == 201 || sort_type == 211)
                ret = comparte(stat1.getPrice(), stat2.getPrice());
            ...
            else if (sort_type == 203 || sort_type == 213)
                ret = comparte(stat1.getAmount(), stat2.getAmount());
            else if (sort_type == 241)
                ret = comparte(stat1.getClosePrev(), stat1.getOpen(), stat2.getClosePrev(), stat2.getOpen());
            else if (sort_type == 242)
                ret = comparte(stat1.getHigh(), stat1.getPrice(), stat2.getHigh(), stat2.getPrice());
            else if (sort_type == 243)
                ret = comparte(stat1.getLow(), stat1.getPrice(), stat2.getLow(), stat2.getPrice());
            else if (sort_type == 244)
                ret = comparte(stat1.getHigh60(), stat1.getPrice(), stat2.getHigh60(), stat2.getPrice());
            else if (sort_type == 245)
                ret = comparte(stat1.getLow60(), stat1.getPrice(), stat2.getLow60(), stat2.getPrice());
            else if (sort_type == 331 || sort_type == 341)
                ret = comparte(stat1.getVolume60(), stat1.getVolume(), stat2.getVolume60(), stat2.getVolume());
            else if (sort_type == 411 || sort_type == 421)
                ret = comparte(stat1.getNet(), stat2.getNet());
            else if (sort_type == 412 || sort_type == 422)
                ret = comparte(stat1.getNetTotal(), stat2.getNetTotal());
            else if (sort_type == 413 || sort_type == 423)
                ret = comparte2(stat1.getGrowth(), stat2.getGrowth());
            else if (sort_type == 414 || sort_type == 424)
                ret = comparte2(stat2.getPremium(), stat1.getPremium());
            else if (sort_type == 415 || sort_type == 425)
                ret = comparte2(stat2.getPremium2(), stat1.getPremium2());
            else if (sort_type == 441 || sort_type == 451)
                ret = comparte2(stat2.getPremiumHigh5(), stat2.getPremium2(), stat1.getPremiumHigh5(), stat1.getPremium2());
            else if (sort_type == 442 || sort_type == 452)
                ret = comparte2(stat2.getPremiumLow5(), stat2.getPremium2(), stat1.getPremiumLow5(), stat1.getPremium2());
        }
    ...
    {
        if (WebUtil.empty(s1) && WebUtil.empty(s2))
            return 0;
        else if (WebUtil.empty(s1))
            return 2;
        else if (WebUtil.empty(s2))
            return -2;
    ...
    {
        if ((base1 <= 0d || base1 == 0d || d1 <= 0d) && (base2 <= 0d || base2 == 0d || d2 <= 0d))
            return 0;
        else if (base1 <= 0d || base1 == 0d || d1 <= 0d)
            return 2;
        else if (base2 <= 0d || base2 == 0d || d2 <= 0d)
            return -2;
    ...
    {
        if ((base1 < -100d || base1 == 0d || d1 < -100d) && (base2 < -100d || base2 == 0d || d2 < -100d))
            return 0;
        else if (base1 < -100d || base1 == 0d || d1 < -100d)
            return 2;
        else if (base2 < -100d || base2 == 0d || d2 < -100d)
            return -2;
        double bias1 = (d1 - base1), bias2 = (d2 - base2);
        return (bias1 == bias2 ? 0 : (bias1 > bias2 ? 1 : -1));
    }
    ...
    public static void appendFundEbk(String code_b, StringBuffer sb)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
            if (i != 1)
                sb.append("0").append(s1.code(i)).append(WebUtil.line());
    }
    ...
    public static void appendFund(int type, String code_b, XmlOutput4j xop)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            if (type == 1)
                arr_nets[i] = getNetDailysList(code);
        }
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            quote =
                    (i == 2 ? null : (type == 1 ? quoteMin + ", " : "") + WebUtil.d2s(4, growth) + ", " + WebUtil.d2s(4, stat.getPrice()) + ", "
                            + WebUtil.d2s(4, stat.getClosePrev()) + ", " + WebUtil.d2s(4, stat.getOpen()) + ", " + WebUtil.d2s(4, stat.getHigh()) + ", "
                            + WebUtil.d2s(4, stat.getLow()) + ", " + WebUtil.d2s(4, stat.getHigh60()) + ", " + WebUtil.d2s(4, stat.getLow60()) + ", "
                            + WebUtil.d2s(4, stat.getVolume()));
            // "date", "net", "netTotal", "growth", "premium", "premium2", "premiumHigh5", "premiumLow5"
            net =
                    stat.getDate() + ", " + WebUtil.d2s(4, stat.getNet()) + ", " + WebUtil.d2s(4, stat.getNetTotal()) + ", " + WebUtil.d2s(4, stat.getGrowth()) + ", "
            ...
            xop.appendTag(false, "stat" + i, null, InVarAM.attrs_rtstats_detail,
                    new String[] { arr_s[i].getCode(), arr_s[i].getName(), arr_s[i].getNameS(), "" + arr_s[i].getType(), WebUtil.d2s(4, arr_s[i].getEquity()), quote, quoteMin,
                            stat.getMinute0(), net, arr_s[i].hqlist(true) });
            if (type == 1 && i > 0)
            {
                xop.openTag("net" + i, null, null);
                for (TNetDaily bean : arr_nets[i])
                    xop.appendTag(
                            false,
                            "net",
                            null,
                            InVarAM.attrs_net,
                            new String[] { "" + DateUtil.date2(bean.getDate()).getTime(), WebUtil.d2s(4, bean.getNet()), WebUtil.d2s(4, bean.getGrowth()),
                                    WebUtil.d2s(4, bean.getPremium()), WebUtil.d2s(4, bean.getPremium2()), WebUtil.d2s(4, bean.getPremiumHigh5()),
                                    WebUtil.d2s(4, bean.getPremiumLow5()) });
                xop.closeTag();
                ...
                for (int ii = 0; ii < min_m; ii++)
                {
                    ...
                    if (stat2 != null)
                        xop.appendTag(
                                ...
                                "net",
                                null,
                                InVarAM.attrs_net_min,
                                new String[] { "" + DateUtil.date(stat2.getDate() + " " + stat2.getMinute0() + ":00").getTime(), WebUtil.d2s(4, stat2.getPremium()),
                                        WebUtil.d2s(4, stat2.getPremium2()), WebUtil.d2s(4, stat2.getPremiumHigh5()), WebUtil.d2s(4, stat2.getPremiumLow5()) });
                }
    ...
    public static void appendSymbolDetail(String code_b, XmlOutput4j xop)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            xop.appendTag(
                    ...
                    "symbol" + i,
                    null,
                    InVarAM.attrs_symbol,
                    new String[] { symbol.getCode(), symbol.getName(), symbol.getNameS(), symbol.getMarket(), "" + symbol.getType(), WebUtil.d2s(4, symbol.getEquity()),
                            symbol.getManager(), symbol.getStartDate(), symbol.getEndDate() });
        }
        xop.closeTag();
    }
    ...
    public static void appendQuoteDetail(String code_b, String date, XmlOutput4j xop)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            {
                if (!s1.code(i).equals(quote_daily.getCode()))
                    continue;
                xop.appendTag(
                        false,
                        "quote" + i,
                        null,
                        InVarAM.attrs_quoteDaily,
                        new String[] { quote_daily.getCode(), quote_daily.getDate(), quote_daily.getTime(), WebUtil.d2s(4, quote_daily.getPrice()),
                                WebUtil.d2s(4, quote_daily.getClosePrev()), WebUtil.d2s(4, quote_daily.getOpen()), WebUtil.d2s(4, quote_daily.getHigh()),
    ...
    public static void appendNetDetail(String code_b, String date, XmlOutput4j xop)
    {
        ...
        for (int i = 0; i < InVarAM.s_names.length; i++)
        {
            ...
            {
                if (!s1.code(i).equals(net_daily.getCode()))
                    continue;
                xop.appendTag(
                        false,
                        "net" + i,
                        null,
                        InVarAM.attrs_netDaily,
                        new String[] { net_daily.getCode(), net_daily.getDate(), WebUtil.d2s(4, net_daily.getNet()), WebUtil.d2s(4, net_daily.getNetTotal()),
                                WebUtil.d2s(4, net_daily.getGrowth()) });
                break;
            }
        }
        xop.closeTag();
