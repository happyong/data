...
public class FundRtStats extends TNetDaily
{
    ...
    public boolean merge2(TQuoteDaily bean)
    {
        ...
        if (diff)
            setClosePrev(bean.getClosePrev());
        ret = ret || diff;
    ...
    {
        if (minute == null || minute.length() < 5)
            return;
        this.minute = minute.substring(0, 5);
        if (this.minute.compareTo(InVarAM.s_trade_times[1]) < 0)
            minute0 = InVarAM.s_trade_times[1];
        else if (this.minute.compareTo(InVarAM.s_trade_times[4]) > 0)
            minute0 = InVarAM.s_trade_times[4];
        else if (this.minute.compareTo(InVarAM.s_trade_times[2]) > 0 && this.minute.compareTo(InVarAM.s_trade_times[3]) < 0)
            minute0 = InVarAM.s_trade_times[2];
        else
            minute0 = this.minute;
    }
