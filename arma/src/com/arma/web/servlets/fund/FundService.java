...
public class FundService extends AbstractBaseComponent
{
            ...
            String processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if ("funds".equals(servlet))
        {
            ...
            for (String code_b : codes_b)
                FundCacher.appendFund(0, code_b, xop);
            xop.closeTag();
            map.put("result", true);
        }
        else if ("fundebk".equals(servlet))
        {
            ...
            for (String code_b : codes_b)
                FundCacher.appendFundEbk(code_b, sb);
        ...
        {
            TSymbol[] beans =
                    new TSymbol[] { new TSymbol().fromRequest(0, null, request), new TSymbol().fromRequest(1, null, request), new TSymbol().fromRequest(2, null, request) };
        ...
        else if ("fundquotedetail".equals(servlet))
        {
            ...
            do
            {
                if (!ArmaUtil.trade(date))
                    break;
                FundCacher.appendQuoteDetail(code_b, date, xop);
                map.put("result", true);
            } while (false);
            if (!(Boolean) map.get("result"))
                map.put("error", date + " is not trade date");
        }
        else if ("fundquoteupdate".equals(servlet))
        {
            ...
            do
            {
                if (bean == null || bean.empty() || !ArmaUtil.trade(bean.getDate()))
                    break;
                FundCacher.updateQuoteDaily(bean);
                map.put("result", true);
            } while (false);
            if (!(Boolean) map.get("result"))
                map.put("error", (bean == null ? "" : bean.getDate()) + " is not trade date");
        }
        ...
        if ((Boolean) map.get("result") && !json)
