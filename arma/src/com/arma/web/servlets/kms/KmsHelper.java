...
import com.arma.web.servlets.kms.bean.KmComparator;
import com.arma.web.servlets.kms.bean.KmKeysComparator;
import com.arma.web.servlets.kms.bean.KmsCond;
import com.arma.web.servlets.kms.bean.KmsIdBuilder;
...
public class KmsHelper
{
    ...
    public static List<Knowledge> listKm(KmsCond cond)
    {
        ...
        if (cond.getKeys().length < 1)
            kmIds = dao.getKmIds(ckeyId, null);
        else
        {
            ...
            for (int ii = 0; ii < keys.length; ii++)
            {
                ...
                if (mkey(tk))
                {
                    ...
                    for (int i = 1; i < arr2.length; i++)
                        tkkCond += " or " + keyCond(false, arr2[i]);
                    tkkCond += (arr2.length > 1 ? ")" : "");
                }
                else
                    tkkCond += keyCond(dkey(tk.getTypeId()), bases[ii]);
    ...
    public static List<Map<String, Object>> listKm4Win(KmsCond cond, ResourceBundle bundle)
    {
        ...
        for (Knowledge km : kms)
            if (km.getTkks() != null)
                for (TKnowkey tkk : km.getTkks())
                    if (!show.contains(tkk.getKeyId()))
                        show.add(tkk.getKeyId());
        for (String str : arr)
            if (show.contains(WebUtil.str2int(str)))
                list.add(KmsCacher.getKey(WebUtil.str2int(str)));
        ...
        for (Knowledge km : kms)
        {
            ...
            if (supdk)
            {
                ...
                for (Keyword key : list)
                    list(index++, none, ckeys, key, tkks, tr);
            }
            else
            {
                list(index++, none, ckeys, cond.getSkey(), tkks, tr);
                for (Keyword key : list)
                    if (sortId != key.getTkey().getKeyId())
                        list(index++, none, ckeys, key, tkks, tr);
                SkyUtil.td(index++, 100, bundle.getString("label_updateDate"), DateUtil.str(tkm.getUpdateDate()), tr);
            }
            ret.add(tr);
    ...
    private static void list(int index, String none, String ckeys, Keyword key, List<TKnowkey> tkks, Map<String, Object> tr)
    {
        ...
        {
            if (tkk.getKeyId() != key.getTkey().getKeyId())
                continue;
    ...
    {
        if (base == null || and.size() < 1)
            return and;
        for (int i = base.size() - 1; i >= 0; i--)
            if (!and.contains(base.get(i)))
                base.remove(i);
        return base;
    }
    ...
    private static List<Knowledge> getKms(List<Integer> kmIds)
    {
        ...
        for (TKnowledge tkm : tkms)
        {
            ...
            if (!map.containsKey(ckeyId))
                map.put(ckeyId, new KmKeysComparator(KmsCacher.getKey(ckeyId).getTkey().getContent()));
        }
        ...
        for (Knowledge bean : ret)
            if (bean.getTkks() != null && map.containsKey(bean.getTkm().getCkeyId()))
                Collections.sort(bean.getTkks(), map.get(bean.getTkm().getCkeyId()));
        return ret;
    }

    private static Knowledge scan0(int kmId, List<Knowledge> kms)
    {
        if (kmId < 1 || kms == null || kms.size() < 1)
            return null;
        for (Knowledge km : kms)
            if (km.getTkm().getKmId() == kmId)
                return km;
        return null;
    }
    ...
    public static int updateKm(boolean onlyKeys, Date now, Knowledge km)
    {
        ...
        {
            if (!onlyKeys)
                b1 = dao.updateKnowledge(km.getTkm());
        ...
        if (km.getTkks() != null && b2)
        {
            ...
            if (!b3)
            {
                ...
                if (sb.length() > 0)
                    _logger.info("error while insert into database" + WebUtil.LINE_WIN + sb.toString());
            }
        }
        return ret;
    }
    ...
    private static String updateKmKeys(int kmIdBase, boolean fmtKeys, List<TKnowkey> newKeys, List<Integer> kmIds)
    {
        List<Knowledge> kms = getKms(kmIds);
        if (kms == null || kms.size() < 1)
            return resp(null, false, null, "km not found");
        ...
        if (!fmtKeys && idmove == 0 && nokeys)
            return resp(null, false, null, "no idmove and no fmtkeys and no newkeys");
        ...
        for (Knowledge km : kms)
        {
            ...
            if (fmtKeys)
            {
                ...
                {
                    if (km.getTkks() == null)
                        km.setTkks(new ArrayList<TKnowkey>());
                    for (TKnowkey key : newKeys)
                        km.getTkks().add(key.copyit());
                ...
                if (builder != null)
                    kmId = builder.kmId(0 - kmIdBase, kmId0);
                km.setKmId(kmId);
                km.updateDate(date);
                km.getTkm().setKmUuid("");
                ...
                if (km.getTkks() != null && b2)
                {
                    ...
                    b3 = dao.insertKnowkeys(params);
                    if (!b3)
                        writesql(kmId0, params, sb);
                    count = params.size();
                    total += count;
                }
                if (!b3)
                {
                    failKmIds += ", " + kmId0 + "��" + kmId;
                    if (builder != null)
                        break;
                }
        ...
        if (sb.length() > 0)
            _logger.info("failed sql statement list" + WebUtil.LINE_WIN + sb.toString());
    ...
    private static Map<String, Object> tkk(TKnowkey bean, List<Map<String, Object>> params)
    {
        ...
        if (key != null && !key.empty() && key.getTkey().isAsEnum())
            KmsCacher.updateKenum(true, bean.getKeyId(), bean.getKeyVal());
        return map;
    }
    ...
    private static void writesql(int kmId, List<Map<String, Object>> params, StringBuffer sb)
    {
        ...
        for (Map<String, Object> map : params)
            sb.append("insert into t_knowkey (km_id, key_id, key_val, update_date) values (").append(map.get("kmId")).append(", ").append(map.get("keyId")).append(", '")
                    .append(map.get("keyVal")).append("', '").append(DateUtil.str((Date) map.get("updateDate"))).append("');").append(WebUtil.LINE_WIN);
    }
    ...
    public static String newKms(int kmIdBase, int kmCount, int ckeyId, HttpServletRequest request)
    {
        ...
        if (kmIdBase < 0)
            return resp(key, false, param, "km id invalid");
        if (kmCount < 1)
            return resp(key, false, param, "km count invalid");
        Keyword ckey = KmsCacher.getKey(ckeyId);
        if (ckey == null || ckey.empty())
            return resp(key, false, param, "ckey not found");
        ...
        {
            if (updateKm(false, new Date(now), new Knowledge().fromRequest(request)) > 0)
                creates++;
    ...
    public static String fmtKmKeys4ckeyId(int ckeyId)
    {
        ...
        if (!WebUtil.empty(invalid))
            return resp(key, false, param, "km ids should be pre-handled manually, kmIds|" + invalid);
        invalid = checkIds(false, ckeyId, dao);
        if (!WebUtil.empty(invalid))
            return resp(key, false, param, "key ids should be pre-handled manually, kkIds|" + invalid);
        ...
        if (deta != 0)
            dao.batchUpdate("update t_knowledge set km_id = km_id + " + deta + " where (km_id between " + min + " and " + max + ") and ckey_id = " + ckeyId, null);
    ...
    private static String checkIds(boolean km, int ckeyId, TKmsDaoService dao)
    {
        ...
        List<Integer> ids =
                (km ? dao.getKmIds(false, "(km_id between " + min + " and " + max + ") and ckey_id != " + ckeyId + " order by km_id") : dao.getKeyIds2("(tkk.id between " + min
                        + " and " + max + ") and tkm.ckey_id != " + ckeyId + " order by tkk.id"));
        String ret = "";
    ...
    private static String content(boolean del, TKnowledge tkm, List<TKnowkey> tkks)
    {
        ...
        for (int i = tkks.size() - 1; i >= 0; i--)
        {
            ...
            if (km_id > tkm.getKmId())
                break;
            else if (km_id < tkm.getKmId())
                continue;
            if (del)
                tkks.remove(i);
            String val = tkk.getKeyVal();
            if (WebUtil.empty(val))
                continue;
            if (ckeyId == 150)
            {
                if (key_id == 1501)
                    ...
                    raddr = (val.length() > 1 ? val.substring(0, 2) : val);
            }
            else if (key_id == tagk())
            {
                if (ckeyId == 120)
                ...
                {
                    if (content.length() < 1 && Character.isDigit(val.charAt(val.length() - 1)))
                        content = val;
                }
            }
        }
    ...
    {
        if (unull)
            return "km_id >= " + kmIdMin + " and km_id <= " + kmIdMax;
        String cond = "1=1";
        if (kmIdMin > 0)
            cond += " and km_id >= " + kmIdMin;
        if (kmIdMax > 0)
            cond += " and km_id <= " + kmIdMax;
        return cond;
    }
    ...
    public static String newKmKey(int keyId, String keyVal, boolean fmtKeys, int kmIdMin, int kmIdMax)
    {
        ...
        if (kk == null || kk.empty())
            return resp(key, false, param, "key not found");
        if (WebUtil.empty(keyVal))
            return resp(key, false, param, "key val invalid");
        if (kmIdMin < 1 || kmIdMax < 1 || kmIdMin > kmIdMax)
            return resp(key, false, param, "km id invalid");
        TKmsDaoService dao = GlobalCache.getInstance().getBean(TKmsDaoService.class);
    ...
    public static String newKmKey4Tags(int keyId, String keyVal, boolean fmtKeys, String tags)
    {
        ...
        if (WebUtil.empty(keyVal))
            return resp(key, false, param, "key val invalid");
        if (WebUtil.empty(tags))
            return resp(key, false, param, "tags invalid");
        TKmsDaoService dao = GlobalCache.getInstance().getBean(TKmsDaoService.class);
        String[] arr = tags.split(InVarAM.s_sep1);
        String cond = "key_id = " + tagk() + " and (";
        for (int i = 0; i < arr.length; i++)
            cond += (i == 0 ? "" : " or ") + "key_val='" + (arr[i].indexOf(InVarAM.s_sep2) == -1 ? arr[i] : arr[i].substring(0, arr[i].indexOf(InVarAM.s_sep2))) + "'";
        cond += ") order by km_id ";
        List<Integer> kmIds = dao.getKmIds(true, cond);
        if (kmIds == null || kmIds.size() < 1)
            return resp(key, false, param, "tags not found");
    ...
    public static String copyKmKeys(int kmIdSrc, int kmIdDest)
    {
        ...
        if (kmIdSrc < 1 || kmIdDest < 1)
            return resp(key, false, param, "km id invalid");
        Knowledge km_src = getKm(kmIdSrc), km_dest = getKm(kmIdDest);
        if (km_src == null)
            return resp(key, false, param, "src km not found");
        if (km_dest == null)
            return resp(key, false, param, "dest km not found");
        if (km_src.getTkm().getCkeyId() != km_dest.getTkm().getCkeyId())
            return resp(key, false, param, "ckey id not match");
        ...
        if (tkks != null)
            Collections.sort(tkks, new KmKeysComparator(ckey.getTkey().getContent()));
    ...
    public static String copyKmKeyDates(int kmIdMin, int kmIdMax)
    {
        ...
        if (tkms == null || tkms.size() < 1)
            return resp(key, false, param, "km not found");

        List<Map<String, Object>> params = new ArrayList<Map<String, Object>>();
        for (TKnowledge tkm : tkms)
            params.add(tkm.toDbMap());
    ...
    public static String resp(String key, boolean pass, String param, String msg)
    {
        ...
        if (!WebUtil.empty(key))
            _logger.info(key + (pass ? ", " : " fail - ") + info);
