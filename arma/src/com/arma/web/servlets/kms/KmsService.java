            ...
            String processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if ("kmlist".equals(servlet))
        {
            ...
            for (Knowledge km : kms)
            {
                ...
                xop.appendTag0(false, "km", null, InVarKM.attrs_km,
                        new String[] { "" + tkm.getKmId(), null, tkm.getContent(), km.getKeys(true), DateUtil.str(tkm.getUpdateDate()) });
            }
            xop.closeTag();
            map.put("result", true);
        }
        ...
        else if ("kmdetail".equals(servlet))
        {
            ...
            if (km != null && !km.empty())
            {
                TKnowledge tkm = km.getTkm();
                // "kmId", "ckeyId", "content", "keys", "updateDate"
                xop.openTag("km", InVarKM.attrs_km,
                        new String[] { "" + tkm.getKmId(), "" + tkm.getCkeyId(), tkm.getContent(), km.getKeys(false), DateUtil.str(tkm.getUpdateDate()) });
                xop.closeTag();
                map.put("result", true);
            }
        }
        ...
        else if ("keydetail".equals(servlet))
        {
            ...
            if (KmsCacher.outputKey(keyId, xop))
                map.put("result", true);
        }
        ...
        else if ("kutil".equals(servlet))
        {
            ...
            if ("coast".equals(method))
                map.put("result", KmsUtil.handleCoast());
            else if ("fleet".equals(method))
                map.put("result", KmsUtil.handleFleet());
            else if ("date".equals(method))
                map.put("result", KmsUtil.handleDate());
            else if ("rocket".equals(method))
                map.put("result", KmsUtil.handleRocket());
            else if (method.startsWith("keyval::"))
                map.put("result", KmsUtil.handleKeyVal(method.substring(8)));
            else if ("vote".equals(method))
                map.put("result", KmsUtil.handleVote());
            else if (method.startsWith("tkktest"))
                map.put("result", KmsUtil.handleTkkTest(WebUtil.str2int(method.substring(7))));
            else if (method.startsWith("finance"))
                map.put("result", KmsUtil.handleFinance(WebUtil.str2int(method.substring(7))));
        }

        if ((Boolean) map.get("result") && !json && usexop)
            map.put("bodyXml", xop.output());
