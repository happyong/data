...
public class PortalFilter implements Filter
{
	...
	public void init(FilterConfig config) throws ServletException 
	{
		...
		if(!WebUtil.empty(str))
		{
			...
			for(int i = 0; i < locales.length; i++)
			{
				...
				if(locale.length==2) supportedLocales.put(locale[0], new Object[]{PortalUtil.parseLocale(locale[0]),locale[1]});
			}
		...
		throws IOException, ServletException 
	{
        ...
        if (no_login && login_nl)
        {
        	...
        	if (WebUtil.empty(user_name)) user_name = "guest";
    		user = UserHelper.userLogin(user_name);
        	if (user == null) user = new UserLogin();
        	if (user.empty()) user.setName(user_name);
