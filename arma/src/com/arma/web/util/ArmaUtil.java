...
public class ArmaUtil
{
    ...
    public static boolean hitfi(String code)
    {
        for (String[] arr : InVarAM.s_fund_include)
            if ((arr.length == 1 && code.equals(arr[0])) || (arr.length > 1 && code.compareTo(arr[0]) >= 0 && code.compareTo(arr[1]) <= 0))
                return true;
        return false;
    }
    ...
    {
        if (WebUtil.empty(base) || WebUtil.empty(to) || base.compareTo(to) >= 0)
            return 0;
        Date d_base = DateUtil.date2(base), d_to = DateUtil.date2(to);
        if (d_base == null || d_to == null)
            return 0;
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            if (hittc(c_base))
                continue;
            days++;
            if (c_base.getTimeInMillis() >= t_to)
                break;
        }
        return days;
    }
    ...
    private static boolean hittc(Calendar cal)
    {
        ...
        for (String[] arr : InVarAM.s_trading_close)
            if ((arr.length == 1 && ret.equals(arr[0])) || (arr.length > 1 && ret.compareTo(arr[0]) >= 0 && ret.compareTo(arr[1]) <= 0))
                return true;
        return false;
    }
