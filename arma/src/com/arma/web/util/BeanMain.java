...
public class BeanMain 
{
	...
	{
		if (WebUtil.empty(file) || WebUtil.empty(xpaths)) return;
		...
		for (int i = 0; i < len; i++)
		{
			...
			if (xpath.length() < xpaths0[index].length()) index = i;
			int pos = xpath.lastIndexOf("/");
			nodes[i] = xpath.substring(pos + 1);
			if (WebUtil.empty(cnames0[i])) cnames0[i] = (nodes[i].substring(0, 1).toUpperCase() + nodes[i].substring(1));
		}
		this.apply = apply;
	...
	{
		if (WebUtil.empty(str)) return new String[len];
	...
	public void beans(boolean req, StringBuffer demo)
	{
		for (int i = 0; i < cnames.length; i++)
		{
			...
			for (BeanMeta meta : attrs) demo.append(meta.constructor());
			for (BeanMeta meta : params) demo.append(meta.constructor());
			if (sbean != null) demo.append(sbean.constructor());
	...
	public void bean(String xpath, String node, String cname, String keyfd)
	{
		scan(xpath, keyfd);
		String empty = empty();
		if (empty.indexOf("WebUtil.empty(") != -1) useweb = true;
		
		StringBuffer sb = new StringBuffer();
		sb.append(head(cname));
		// generate field
		if (message) sb.append(blanks(1)).append("public static final String TAG_NAME = \"").append(node).append("\";").append(WebUtil.line())
			.append(WebUtil.line());
		for (BeanMeta meta : attrs) sb.append(meta.param());
		for (BeanMeta meta : params) sb.append(meta.param());
		if (sbean != null) sb.append(sbean.param());
		...
		for (BeanMeta meta : attrs) sb.append(meta.copyit());
		for (BeanMeta meta : params) sb.append(meta.copyit());
		if (sbean != null) sb.append(sbean.copyit());
		...
		for (BeanMeta meta : attrs) sb.append(meta.fromNode(true));
		for (BeanMeta meta : params) sb.append(meta.fromNode(false));
		if (sbean != null) sb.append(blanks(2)).append("if (node != null) ").append(sbean.getName2()).append(" = node.getText();").append(WebUtil.line());
		if (message && !nlss)
		{
		...
		for (BeanMeta meta : params) sb.append(meta.addParam());
		...
		String str = (WebUtil.empty(names) ? "null, null" : "new String[]{" + names.substring(2) + "}, " + 
				(attrs.size() > 3 ? WebUtil.line() + blanks(3) : "") + "new String[]{" + values.substring(2) + "}");
		String tag = (message ? "TAG_NAME" : "\"" + node + "\""), xpath0 = (message ? "\"//\" + TAG_NAME" : "\"" + xpath + "\"");
		if (single)
		{
			sb.append(blanks(2)).append("xop.appendTag(").append(sbean != null && sbean.isCdata() ? "true" : "false").append(", ").append(tag)
				.append(", ").append(sbean != null ? sbean.getName2() : "null").append(", ").append(str).append(");").append(WebUtil.line());
		}
		else
		{
			sb.append(blanks(2)).append("xop.openTag(").append(tag).append(", ").append(str).append(");").append(WebUtil.line());
			for (BeanMeta meta : params) sb.append(meta.toXop());
		...
		if (message)
		{
			...
			sb.append(blanks(2)).append(cname).append(" bean = new ").append(cname).append("().fromNode(ParseUtil.parseXml0(xml, ").append(xpath0)
			...
			if (!nlss)
			{
				...
				if (false && subtype) sb.append(blanks(2)).append("bean.setSubType(getSubType());").append(WebUtil.line());
		...
		for (BeanMeta meta : attrs) sb.append(meta.getset());
		for (BeanMeta meta : params) sb.append(meta.getset());
		if (sbean != null) sb.append(sbean.getset());
		// sb.append(WebUtil.line());
		...
		if (apply) FileUtil.writeFile(content, dir_java + file);
	...
	private void scan(String xpath, String keyfd)
	{
		...
		for (int i = 0; i < count; i++) add(false, false, ele.attribute(i).getName(), null, attrs);
		...
		if (list == null || list.size() == 0)
		{
			...
			single = true;
			if (!WebUtil.empty(ele.getText())) sbean = new BeanMeta(ParseUtil.isCDATA(ele), "String", "value");
		}
		else
		{
			if (same(list)) add(false, true, list.get(0).getName(), null, params);
			else for (Element e : list) add(ParseUtil.isCDATA(e), false, e.getName(), e, params);
		}
	...
	private boolean same(List<Element> list)
	{
		...
		for (Element e : list) if (!e.getName().equals(name)) return false;
		return true;
	}
	...
	private void add(boolean cdata, boolean blist, String name, Element e, List<BeanMeta> list)
	{
		...
		if (list_cls != null && list_xpath != null) m.extend(list_cls, list_xpath);
		for (BeanMeta meta : list) if (name.equals(meta.getName())) return;
		System.out.println("add param, " + name + "|" + m.toText());
		if ("subType".equals(m.getName2())) subtype = true;
		list.add(m);
	}
	
	private int end(String key, String[] arr)
	{
		for (int i = 0; i < arr.length; i++) if (arr[i].endsWith(key)) return i;
		return -1;
	}
	...
	{
		String expr = "";
		for (BeanMeta meta : attrs) if (hit(meta.getName(), keys) != -1) expr += meta.empty(keyop);
		for (BeanMeta meta : params) if (hit(meta.getName(), keys) != -1) expr +=  meta.empty(keyop);
		if (sbean != null && hit(sbean.getName(), keys) != -1) expr +=  sbean.empty(keyop);
	...
	private String head(String cname)
	{
		...
		sb.append(WebUtil.line());
		if (uselist) sb.append("import java.util.ArrayList;").append(WebUtil.line());
		if (usedate || (message && !nlss)) sb.append("import java.util.Date;").append(WebUtil.line());
		if (uselist) sb.append("import java.util.List;").append(WebUtil.line());
		if (usedate || (message && !nlss) || uselist) sb.append(WebUtil.line());
		...
		if (useweb || (message && !nlss)) sb.append("import com.neulion.iptv.web.util.WebUtil;").append(WebUtil.line());
		sb.append("import com.neulion.iptv.web.util.XmlOutput4j;").append(WebUtil.line());
		if (usedate) sb.append("import com.neulion.nlss.message.APIUtil;").append(WebUtil.line());
		if (message && !nlss) sb.append("import com.neulion.nlss.message.service.bean.TRequests;").append(WebUtil.line());
		sb.append(WebUtil.line());
	...
	private static void demo(String append, boolean apply, String api, StringBuffer demo, BeanMain request, BeanMain response)
	{
		...
		for (String str : request.cnames) list.add(str);
		for (String str : response.cnames) list.add(str);
		Collections.sort(list);
		for (String str : list) sb.append("import com.neulion.nlss.message.bean.").append(str).append(";").append(WebUtil.line());
		sb.append(WebUtil.line());
		...
		if (count > 1)
		{
			...
			if (i1 != -1 && i2 != -1) 
			{
				...
				for (int i = 1; i < count; i++) sb.append(s).append((i1 + i) + e1).append("\", \"").append((i2 + i) + e2).append(e);
			}
		}
		else if (append.startsWith("req:"))
		{
			String[] arr = append.substring(4).split(",");
			for (String str : arr) sb.append(s).append(str).append("\", \"").append(fresp).append(e);
		}
		else if (append.startsWith("resp:"))
		{
			String[] arr = append.substring(5).split(",");
			for (String str : arr) sb.append(s).append(freq).append("\", \"").append(str).append(e);
		}
		sb.append(blanks(1)).append("}").append(WebUtil.line());
	...
	private static void util(BeanMain request, BeanMain response)
	{
		...
		body += blanks(2) + "return (xml.indexOf(\"<\" + " + req_mcname + ".TAG_NAME + \" \") != -1" + 
		...
		body += blanks(1) + "{" + WebUtil.line();
		body += blanks(2) + "return (xml.indexOf(\"<\" + " + resp_mcname + ".TAG_NAME + \" \") != -1" + 
	...
	private static void api(String head, String body, String tail, StringBuffer sb)
	{
		...
		if (pos1 != -1 && pos2 != -1) sb.replace(pos1, pos2, head + body);
		else sb.append(head).append(body).append(tail);
	...
	private static void output(boolean req, boolean in, String cls, StringBuffer sb)
	{
		...
		sb.append(blanks(2)).append("String ").append(var).append(" = new File(dir_").append(in0).append(" + ").append(req0)
			.append("_file).getAbsolutePath();").append(WebUtil.line());
	...
	{
		if (WebUtil.empty(str)) return new String[]{"", ""};
		int pos = str.lastIndexOf("|");
		return pos == -1 ? new String[]{str, ""} : new String[]{str.substring(0, pos), str.substring(pos + 1)};
	}
	
	private static void generate(boolean apply, String api, String files, String xpaths, String attrs, String cnames, String keyfds, 
			String boolfds, String intfds, String datefds, String append)
	{
	...
	private static String[][] api_all = new String[][]
	{
		// api, files, xpaths, attrs, cnames, keyfds, boolfds, intfds, datefds, append
		{
			...
			"//updateLiveSchedule,//updateLiveSchedule/programs/program,//updateLiveSchedule/programs/program/streams/stream|//updateLiveScheduleResponse", 
			"close,|,nlss", "UpdateLiveScheduleRequest,LiveScheduleProgram,LiveScheduleStream|", "client#name,programId+type,|client+nlssId", 
			"cc|", "season,gameType,stopEncoderAfter,programId,numberOfCameras|rc", "beginDate,endDate,beginDatetime,endDatetime|", "2"
		}, {
			"deleteLiveSchedule", "3.deleteLiveSchedule.xml|3.deleteLiveScheduleResponse.xml", 
			"//deleteLiveSchedule|//deleteLiveScheduleResponse", 
			"close,|,nlss", "DeleteLiveScheduleRequest|", "client|client+nlssId", 
			"", "programId|rc", "", ""
		}, {
			"endLiveSchedule", "4.endLiveSchedule.xml|4.endLiveScheduleResponse.xml", 
			"//endLiveSchedule|//endLiveScheduleResponse", 
			"close,|,nlss", "EndLiveScheduleRequest|", "client|client+nlssId", 
			"", "programId|rc", "", ""
		}, {
			"syncProgram", "5.syncProgram.xml|5.syncProgramResponse.xml", 
			"//syncProgram|//syncProgramResponse", 
			"close,|,nlss", "SyncProgramRequest|", "client|client+nlssId", 
			"", "programId|rc", "", ""
		}, {
			"updateLiveProgram", "6.updateLiveProgram.xml|6.updateLiveProgramResponse.xml", 
			"//updateLiveProgram,//updateLiveProgram/programs/program,//updateLiveProgram/programs/program/pushUrl,"
					+ "//updateLiveProgram/programs/program/playbackUrls/playbackUrl|//updateLiveProgramResponse", 
			"close,nlss|,", "UpdateLiveProgramRequest,LiveProgram,LivePushUrl,LivePlaybackUrl|", "client,programId,url,device+url|client+nlssId", 
			"", "programId,bitrate|rc", "", "2"
		}, {
			"updateArchiveProgram", "8.updateArchiveProgram.xml|8.updateArchiveProgramResponse.xml", 
			"//updateArchiveProgram,//updateArchiveProgram/playbackUrls/playbackUrl|//updateArchiveProgramResponse", 
			"close,nlss|,", "UpdateArchiveProgramRequest,LiveAdmuxPlaybackUrl|", "client,device+url|client+nlssId", 
			"additionalAudio|", "programId,cmsProgramId,duration,bitrate|rc,cmsProgramId", "", ""
		}, {
			"updateHighlightProgram", "9.updateHighlightProgram.xml|9.updateHighlightProgramResponse.xml", 
			"//updateHighlightProgram,//updateHighlightProgram/playbackUrls/playbackUrl|//updateHighlightProgramResponse", 
			"close,nlss|,", "UpdateHighlightProgramRequest,LiveAdmuxPlaybackUrl|", "client,device+url|client+nlssId", 
			"additionalAudio|", "programId,cmsProgramId,duration,bitrate|rc,cmsProgramId", "", ""
		}, {
			"vodRequest", "10.vodRequest.xml|10.vodStatusResponse.xml", 
			"//vodRequest,//vodRequest/devices/device,//vodRequest/profiles,//vodRequest/profiles/profile,"
					+ "//vodRequest/sources/source|//vodStatusResponse,//vodStatusResponse/playbackUrls/playbackUrl", 
			",|,nlss", ",VodDevice,VodProfiles,VodProfile,VodSource|,VodPlaybackUrl", "client+id,name+protocol,name+profile,name,"
					+ "http_file#unc_file|client+id,device+url", 
	...
	public static void main(String[] args)
	{
        ...
        if ("api_all".equals(api))
    		// api, files, xpaths, attrs, cnames, keyfds, boolfds, intfds, datefds, append
        	for (int i = 0; i < api_all.length; i++) generate(apply, api_all[i][0], api_all[i][1], api_all[i][2], api_all[i][3], api_all[i][4], 
        			api_all[i][5], api_all[i][6], api_all[i][7], api_all[i][8], api_all[i][9]);
        else
