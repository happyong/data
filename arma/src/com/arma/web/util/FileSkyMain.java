...
{   
	...
	public static void main(final String[] args)
	{
		...
		CommandLine cl = null;
        try { cl = parser.parse(opts, args); } catch (ParseException e) 
        { 
	...
	{
		if (new File(work, metas_file).exists()) return;
		...
		for (File file : files)
		{
			if (rubbish(file)) continue; 
			if (meta_file.equals(file.getName())) continue;
			if (file.isDirectory())
			{
				if (recursive) simple(revert, recursive, base, file);
				continue;
			}
	...
	{
		if (new File(base, meta_file).exists()) return;
	...
	// FileSkyMain -base [-revert] -merges
	private static void merges(boolean revert, File base, File work, Map<String, Integer> metas, 
			Map<String, FileSkyMeta> map_cur, Map<String, FileSkyMeta> map_all)
	{
		if (new File(work, meta_file).exists()) return;
		
		File[] files = work.listFiles();
		if (files == null) files = new File[0];
		Arrays.sort(files);
		String subfolder = work.getAbsolutePath().substring(base.getAbsolutePath().length());
		if (!WebUtil.empty(subfolder)) subfolder = subfolder.substring(1) + File.separator;
		String prefix = (WebUtil.empty(subfolder) ? work : work.getParentFile()).getName() + ".";
		if (WebUtil.cn(prefix)) prefix = WebUtil.md5(WebUtil.bytes2hexs(WebUtil.str2bytes(prefix, null))).substring(0, 6) + ".";
		for (File file : files)
		{
			if (rubbish(file)) continue; 
			if (metas_file.equals(file.getName())) 
			{
				if (!revert) FileUtil2.delete(file.getAbsolutePath());
				continue;
			}
			if (file.isDirectory())
			{
		...
		if (!revert && !WebUtil.empty(subfolder)) 
		{
			...
			if (files == null || files.length < 1) FileUtil2.delete(work.getAbsolutePath());
		}
	}
	
	private static void rename(boolean revert, String prefix, String subfolder, File file, File work, Map<String, Integer> metas, 
			Map<String, FileSkyMeta> map_cur, Map<String, FileSkyMeta> map_all)
	{
		String dest = "";		
		...
		if (WebUtil.empty(dest) || meta.isRevived()) return;
		File df = new File(work.getAbsolutePath() + File.separator + dest);
		if (df.exists()) 
			file.delete();
		else 
		{
			if (!df.getParentFile().exists()) FileUtil2.createDirectory(df.getParent());
			if (revert && !meta.isFlat() && !new File(df.getParentFile(), metas_file).exists())
			{
				...
				if (!WebUtil.empty(subfolder)) subfolder = subfolder.substring(1);
	...
	{
		if (!"Thumbs.db".equalsIgnoreCase(file.getName())) return false;
		FileUtil2.delete(file.getAbsolutePath());
        System.out.println("delete rubbish: " + file.getAbsolutePath());
        return true;
	}
	
	private static void load(boolean merges, boolean revert, File work, List<Node> nodes, Map<String, Integer> metas, 
			Map<String, FileSkyMeta> map_cur, Map<String, FileSkyMeta> map_all)
	{
