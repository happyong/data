...
public class FileSkyMeta
{
    ...
    {
        if (deep < 1 || deep > FileSkyMain.max_sub_level || deep > name0.length - 1)
            return "";
        String path = path(deep);
        if (WebUtil.empty(path))
            return "";
    ...
    {
        if (name0 == null || name0.length < 1 || deep < 1 || deep > name0.length)
            return "";
        String path = "";
        for (int i = 0; i < deep; i++)
            path += File.separator + name0[i];
    ...
    private void setName0(String str)
    {
        ...
        if (one != -1)
        {
            ...
            // [1, 4] is " ", max 4 " "
            for (; i < 5; i++)
                if (name0.length() > i && name0.charAt(i) != ' ')
                    break;
            if (i != 1)
                this.level = one + "" + (5 - i);
            else
