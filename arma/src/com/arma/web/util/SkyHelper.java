...
public class SkyHelper
{
    ...
    public static boolean sky(String servlet, HttpServletRequest request, HttpServletResponse response)
    {
        ...
        {
            html = (WebUtil.empty(p1) ? SkyUtil.getHtml4SqlPage(address) : SkyUtil.getHtml4SqlContent(p1, address, GlobalCache.getInstance().getBean(ConfigDaoService.class)));
            success = true;
        }
            ...
            else
            {
                if (p1.startsWith("{armalog}"))
                {
                    ...
                    if (FileUtil.exist(1, log) == null)
                        log = WebUtil.path(GlobalCache._config_root + "/../logs/arma.log");
