...
public class TableMain
{
    ...
    public static final String[][] ss = new String[][] { new String[] { "2015 NeuLion", "com.neulion.iptv.web.", "servlets.admin.", "InVarA", "WebUtil.obj2double(" },
    ...
    private static void spring()
    {
        ...
        if (usedate || usedate2)
            sb.append("import com.neulion.iptv.web.util.DateUtil;").append(WebUtil.line());
        ...
        for (TableMeta meta : params)
            if (!meta.isAutoincr())
                sb.append(blanks(2)).append(meta.toDbMap()).append(WebUtil.line());
        sb.append(blanks(2)).append("return dbmap;").append(WebUtil.line());
        sb.append(mtail());
        // generate fromDbMap method
        sb.append(mhead(true, "@SuppressWarnings(\"unchecked\")" + WebUtil.line() + blanks(1) + "public <T> T fromDbMap(Class<T> cls, Map<String, Object> dbmap)"));
        sb.append(blanks(2)).append("if (dbmap == null || dbmap.size() == 0) return (T)this;").append(WebUtil.line());
        for (TableMeta meta : params)
            sb.append(blanks(2)).append(meta.fromDbMap(0)).append(WebUtil.line());
        ...
        for (TableMeta meta : params)
            if (!meta.isAutoincr())
                sb.append(blanks(2)).append(meta.fromDbMap(2)).append(WebUtil.line());
        ...
        if (apply)
            FileUtil.writeFile(content, dir_java_spring + file);
    ...
    private static String crud(String type)
    {
        ...
        if (!"select".equals(type))
        {
            ...
            else if ("update".equals(type))
            {
                ...
                {
                    if (hit(meta.getName(), keys) != -1)
                        p0.append(" and ").append(meta.getName()).append("=:").append(meta.getName2());
                    else if (!meta.isAutoincr())
                        p1.append(", ").append(meta.getName()).append("=:").append(meta.getName2());
                }
        ...
        else
        {
            ...
            sql.append("select ");
            for (TableMeta meta : params)
                p0.append(", ").append(meta.getName()).append(" as ").append(meta.getName2());
    ...
    private static String text()
    {
        ...
        for (TableMeta meta : params)
            expr += meta.text();
        if (!WebUtil.empty(expr))
            expr = expr.substring(head.length());
    ...
    private static void savanna()
    {
        ...
        for (TableMeta meta : params)
            sb.append(blanks(2)).append(meta.fromDbMap(1)).append(WebUtil.line());
        sb.append(blanks(2)).append("return this;").append(WebUtil.line());
        sb.append(mtail());
        // generate copyit method
        sb.append(mhead(false, "public " + utbl + " copyit()"));
        ...
        FileUtil.writeFile(content, dir_out + file1);
        if (apply)
            FileUtil.writeFile(content, dir_java_savanna + file);
        System.out.println(content);
    }
    ...
    private static String cruds(String type)
    {
        ...
        if (!"select".equals(type))
        {
            ...
            sb.append(blanks(2)).append("if (bean == null").append("delete".equals(type) ? "" : " || bean.empty()").append(") return 0;").append(WebUtil.line());
    ...
    private static String[][] table_all = new String[][] {
            ...
            /*
             * {"t_symbol", "code", "", ""}, {"t_net_daily", "code+date", "", ""}, {"t_quote_daily", "code+date", "", ""}
             */
            { "t_keytype", "type_id", "", "" }, { "t_keyenum", "key_id", "", "" }, { "t_keyword", "key_id", "", "as_enum" }, { "t_keyrank", "key_id", "", "" },
            { "t_knowledge", "km_id", "", "" }, { "t_knowkey", "id", "", "" } };
    ...
    public static void main(String[] args)
    {
        ...
        if ("table_all".equals(table))
        {
            ...
            // table, keyfds, blackfds, boolfds
            for (int i = 0; i < table_all.length; i++)
                generate(apply, script, type, table_all[i][0], table_all[i][1], table_all[i][2], table_all[i][3]);
        }
        else
            generate(apply, script, type, table, (cl.hasOption("keyfds") ? cl.getOptionValue("keyfds") : ""), (cl.hasOption("blackfds") ? cl.getOptionValue("blackfds") : ""),
                    (cl.hasOption("boolfds") ? cl.getOptionValue("boolfds") : ""));

        if ("savanna".equals(type))
        {
            String content = sb_dao.toString(), dir_out_bk1 = dir_out + "_bk1", dir_out_bk2 = dir_out + "_bk2";
            String file = ("table_all".equals(table) ? name : utbl) + "Dao.java", file1 = "/savanna/" + file, bk0 = FileUtil.readFile(dir_out + file1), bk1 =
                    FileUtil.readFile(dir_out_bk1 + file1);
            FileUtil.writeFile((WebUtil.empty(bk1) ? (WebUtil.empty(bk0) ? content : bk0) : bk1), dir_out_bk2 + file1);
            FileUtil.writeFile((WebUtil.empty(bk0) ? content : bk0), dir_out_bk1 + file1);
            FileUtil.writeFile(content, dir_out + file1);
        }
        ...
        {
            String content = sb_dao.toString(), dir_out_bk1 = dir_out + "_bk1", dir_out_bk2 = dir_out + "_bk2";
            String file = ("table_all".equals(table) ? name : utbl) + "Dao.java", file1 = "/spring/" + file, bk0 = FileUtil.readFile(dir_out + file1), bk1 =
                    FileUtil.readFile(dir_out_bk1 + file1);
            FileUtil.writeFile((WebUtil.empty(bk1) ? (WebUtil.empty(bk0) ? content : bk0) : bk1), dir_out_bk2 + file1);
            FileUtil.writeFile((WebUtil.empty(bk0) ? content : bk0), dir_out_bk1 + file1);
            FileUtil.writeFile(content, dir_out + file1);
    ...
    private static void generate(boolean apply_b, String script, String type, String table, String keyfds, String blackfds, String boolfds)
    {
        ...
        if (script.indexOf("\\") == -1 && script.indexOf("/") == -1)
            script = ("savanna".equals(type) ? dir_script_savanna : dir_script_spring) + script;
    ...
    private static boolean readList(String list, List<String> rets)
    {
        ...
        if (WebUtil.empty(list) || rets == null)
            return useindex;
        rets.clear();
        ...
        if (rets.size() == 0)
        {
            useindex = false;
            for (String str : arr)
                if (str.length() > 0 && !rets.contains(str))
                    rets.add(str);
        }
        return useindex;
    }
