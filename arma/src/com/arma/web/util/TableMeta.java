...
public class TableMeta
{
    ...
    public int init(String line, int blackindex, int boolindex, List<String> blackfds, List<String> boolfds)
    {
        ...
        else if (line2l.startsWith("int") || line2l.startsWith("tinyint") || line2l.startsWith("smallint"))
        {
            ...
            if (defval != null)
            {
                if ("int".equals(type) && !"0".equals(defval))
                    def = defval;
                if ("boolean".equals(type) && "1".equals(defval))
                    def = "true";
            }
        }
        else if (line2l.startsWith("float") || line2l.startsWith("double") || line2l.startsWith("decimal"))
        {
            type = "double";
            String defval = defval(line2, line2l);
            if (defval != null && !"0".equals(defval))
                def = defval;
        }
        else
        {
        ...
        if (line2l.indexOf("auto_increment") != -1)
            autoincr = true;
    ...
    public String param()
    {
        ...
        if (def != null)
            sb.append(" = ").append("String".equals(type) ? "\"" + def + "\"" : def);
    ...
    public String getset()
    {
        ...
        sb.append(TableMain.blanks(1)).append("public void set").append(name3).append("(").append(type).append(" ").append(name2).append(") {").append(WebUtil.line());
